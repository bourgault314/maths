...
\begin{tcolorbox}[colback=white, colframe=black, title=\centering\textbf{GABARIT : ENQUÊTE MULTIPLICATIVE À DEUX INCONNUES (cas \texttimes\ \k)}]
    ...
    \begin{tikzpicture}[scale=0.80, every node/.style={font=\bfseries\small}]
        ...
        \path[use as bounding box] (0, 7.2) rectangle (\W, -27.0);
        \def\Xstep{-3.5}
...
% ==================================================
% 3 inconnues (cas ×n et ×m) — VERSION FINALE AÉRÉE
% ==================================================
...
\centering
\begin{tikzpicture}[scale=0.80, every node/.style={font=\bfseries\small}]
    % Cadre élargi à -24.0 vers le bas !
    \path[use as bounding box] (0, 8.8) rectangle (\W, -25.5);
    \def\Xstep{-3.5}

    % =========================
    % ÉTAPE 1 : REPRÉSENTER
    % =========================
    \node[overlay, anchor=west, blue!80!black] at (\Xstep, 8.2) {\large ÉTAPE 1 : REPRÉSENTER (Je dessine)};
    
    % 1ère part
    \node[overlay, anchor=east] at (-0.2, 6.9) {1\textsuperscript{re} part : ....................};
    \draw[thick, fill=mypink!30] (0, 6.4) rectangle (\U, 7.4);
    \node at ({\U/2}, 6.9) {\Large ?};

    % 2e part
    \node[overlay, anchor=east] at (-0.2, 4.8) {2e\ part : ....................};
    \draw[thick, fill=mypink!30] (0, 4.3) rectangle (\WbigB, 5.3);
    \draw[dashed, gray!60, thick] (\U, 4.3) -- (\U, 5.3);
    \draw[dashed, gray!60, thick] ({\U+\dotsWB}, 4.3) -- ({\U+\dotsWB}, 5.3);
    \node at ({\U/2}, 4.8) {\Large ?};
    \node at ({\U+\dotsWB/2}, 4.8) {\Large $\cdots$};
    \node at ({\U+\dotsWB+\U/2}, 4.8) {\Large ?};
    \draw [decorate, decoration={brace, amplitude=6pt, mirror}, thick]
        (0, 4.1) -- (\WbigB, 4.1) node [midway, below=8pt] {\Large ? $\times$ ....................};

    % 3e part
    \node[overlay, anchor=east] at (-0.2, 2.1) {3e\ part : ....................};
    \draw[thick, fill=mypink!30] (0, 1.6) rectangle (\WbigC, 2.6);
    \draw[dashed, gray!60, thick] (\U, 1.6) -- (\U, 2.6);
    \draw[dashed, gray!60, thick] ({\U+\dotsWC}, 1.6) -- ({\U+\dotsWC}, 2.6);
    \node at ({\U/2}, 2.1) {\Large ?};
    \node at ({\U+\dotsWC/2}, 2.1) {\Large $\cdots$}; 
    \node at ({\U+\dotsWC+\U/2}, 2.1) {\Large ?};
    \draw [decorate, decoration={brace, amplitude=6pt, mirror}, thick]
        (0, 1.4) -- (\WbigC, 1.4) node [midway, below=8pt] {\Large ? $\times$ ....................};

    % Accolade TOTAL
    \draw [decorate, decoration={brace, amplitude=12pt, mirror}, thick]
        ({\WbigC+0.3}, 1.4) -- ({\WbigC+0.3}, 7.4) node [midway, right=15pt] {\textbf{TOTAL = ..........}};

    % =========================
    % ÉTAPE 2 : VISUALISER
    % =========================
    % Descendu à -0.5 pour plus d'espace SOUS la 3e part
    \node[overlay, anchor=west, blue!80!black] at (\Xstep, -0.5) {\large ÉTAPE 2 : VISUALISER (J'aligne tout)};
    
    % Total orange (suit la descente)
    \draw[thick, fill=myorange!50] (0, -2.7) rectangle (\W, -1.7);
    \node at ({\W/2}, -2.2) {\large ....................};

    % Alignement rose
    \draw[thick, fill=mypink!30] (0, -3.7) rectangle (\W, -2.7);
    % Séparations
    \foreach \x in {1.2, 2.4, 3.6, 4.8, 6.0, 8.4} {
        \draw[dashed, gray!60, thick] (\x, -3.7) -- (\x, -2.7);
    }
    
    \node at (0.6, -3.2) {\Large ?};
    \node at (1.8, -3.2) {\Large ?};
    \node at (3.0, -3.2) {\Large $\cdots$};
    \node at (4.2, -3.2) {\Large ?};
    \node at (5.4, -3.2) {\Large ?};
    \node at (7.2, -3.2) {\Large $\cdots$};
    \node at (9.0, -3.2) {\Large ?};

    \draw [decorate, decoration={brace, amplitude=5pt, mirror}, thick] (0, -3.9) -- (1.2, -3.9) node [midway, below=4pt] {1\textsuperscript{re} part};
    \draw [decorate, decoration={brace, amplitude=5pt, mirror}, thick] (1.2, -3.9) -- (4.8, -3.9) node [midway, below=4pt] {2e\ part};
    \draw [decorate, decoration={brace, amplitude=5pt, mirror}, thick] (4.8, -3.9) -- (9.6, -3.9) node [midway, below=4pt] {3e\ part};

    % =========================
    % ÉTAPE 3 : CALCULER
    % =========================
    % Descendu à -5.8 pour plus d'espace SOUS le tableau (accolades) de l'Étape 2
    \node[overlay, anchor=west, blue!80!black] at (\Xstep, -5.8) {\large ÉTAPE 3 : CALCULER (Je divise le total en ....)};
    
    \draw[thick, fill=myorange!50] (0, -8.0) rectangle (\W, -7.0);
    \draw[thick, fill=mypink!30] (0, -9.0) rectangle (\W, -8.0);
    
    \foreach \x in {1.2, 2.4, 3.6, 4.8, 6.0, 8.4} {
        \draw[dashed, gray!60, thick] (\x, -9.0) -- (\x, -7.0);
        \draw[dashed, ultra thick, blue] (\x, -6.8) -- (\x, -9.2); 
    }
    % Textes orange
    \node at (0.6, -7.5) {.....};
    \node at (1.8, -7.5) {.....};
    \node at (3.0, -7.5) {\Large $\cdots$};
    \node at (4.2, -7.5) {.....};
    \node at (5.4, -7.5) {.....};
    \node at (7.2, -7.5) {\Large $\cdots$};
    \node at (9.0, -7.5) {.....};
    % Textes rose
    \node at (0.6, -8.5) {\Large ?};
    \node at (1.8, -8.5) {\Large ?};
    \node at (3.0, -8.5) {\Large $\cdots$};
    \node at (4.2, -8.5) {\Large ?};
    \node at (5.4, -8.5) {\Large ?};
    \node at (7.2, -8.5) {\Large $\cdots$};
    \node at (9.0, -8.5) {\Large ?};

    % =========================
    % ÉTAPE 4 : CALCULER
    % =========================
    \node[overlay, anchor=west, blue!80!black] at (\Xstep, -10.6) {\large ÉTAPE 4 : CALCULER (Je trouve les trois parts)};
    
    % Bloc regroupé orange
    \draw[thick, fill=myorange!50] (0, -12.8) rectangle (\W, -11.8);
    \draw[thick] (1.2, -12.8) -- (1.2, -11.8);
    \draw[thick] (4.8, -12.8) -- (4.8, -11.8);
    \node at (0.6, -12.3) {.....};
    \node at (3.0, -12.3) {..........};
    \node at (7.2, -12.3) {...............};

    % Bloc rose découpé
    \draw[thick, fill=mypink!30] (0, -13.8) rectangle (\W, -12.8);
    \foreach \x in {1.2, 2.4, 3.6, 4.8, 6.0, 8.4} {
        \draw[dashed, gray!60, thick] (\x, -13.8) -- (\x, -12.8);
    }
    \node at (0.6, -13.3) {\Large ?};
    \node at (1.8, -13.3) {\Large ?};
    \node at (3.0, -13.3) {\Large $\cdots$};
    \node at (4.2, -13.3) {\Large ?};
    \node at (5.4, -13.3) {\Large ?};
    \node at (7.2, -13.3) {\Large $\cdots$};
    \node at (9.0, -13.3) {\Large ?};

    \draw [decorate, decoration={brace, amplitude=5pt, mirror}, thick] (0, -14.0) -- (1.2, -14.0) node [midway, below=4pt] {1\textsuperscript{re} part};
    \draw [decorate, decoration={brace, amplitude=5pt, mirror}, thick] (1.2, -14.0) -- (4.8, -14.0) node [midway, below=4pt] {2e\ part};
    \draw [decorate, decoration={brace, amplitude=5pt, mirror}, thick] (4.8, -14.0) -- (9.6, -14.0) node [midway, below=4pt] {3e\ part};

    % =========================
    % RÉPONSES ET VÉRIFICATION
    % =========================
    \node[draw=black, thick, rounded corners, fill=yellow!20, inner sep=10pt, align=left] at ({\W/2}, -17.0) {
        \large \textbf{RÉPONSES :} \\[0.2cm]
        1\textsuperscript{re} part : ..................... \quad 2e part : ..................... \quad 3e part : .....................
    };

    \node[draw=green!50!black, dashed, thick, rounded corners, inner sep=10pt, align=left] at ({\W/2}, -21.5) {
        \large \textbf{JE VÉRIFIE (OBLIGATOIRE) :} \\[0.3cm]
        \large 1. Somme : ........ + ........ + ........ = \textbf{.......... (Total ?)} \quad $\square$ OK \\[0.2cm]
...
\begin{tikzpicture}[remember picture,overlay]
    \node[anchor=south west, inner sep=0pt] at ([xshift=0.55cm,yshift=0.22cm]current page.south west)
