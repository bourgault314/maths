...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \ifnum\value{page}=2\relax
    \node[anchor=south east,inner sep=0pt,text=brandNavy!70,font=\sffamily\fontsize{6.8}{6.8}\selectfont]
...
\newcommand{\BigCheckBox}{\tikz[baseline=-0.62ex]{\draw[rounded corners=0pt,brandNavy,line width=0.75pt] (0,0) rectangle (0.38,0.38);}\hspace{0.08cm}}
\newcommand{\LowBigCheckBox}{\tikz[baseline=0.85ex]{\draw[rounded corners=0pt,brandNavy,line width=0.75pt] (0,0) rectangle (0.38,0.38);}\hspace{0.08cm}}
...
\newcommand{\Header}[2]{%
\begin{center}
{\fontsize{19.4}{20.5}\selectfont\bfseries\color{brandNavy} #1}\par\vspace{0.03cm}
{\fontsize{12.7}{13.5}\selectfont\bfseries\color{brandTeal} #2}
...
\begin{tikzpicture}[scale=#1,line cap=butt,line join=miter]
  ...
  \draw[brandSky!65,line width=2.35pt] (A)--(B);
  \draw[brandSky!65,line width=2.35pt] (A)--(C);
  \draw[brandSky,line width=1.25pt] (A)--(B);
  \draw[brandSky,line width=1.25pt] (A)--(C);
  \draw[brandNavy,line width=1.25pt] (A)--(M);
  \draw[brandNavy,line width=1.25pt] (A)--(N);
  \draw[brandNavy,line width=1.28pt] (M)--(N);
...
\begin{tikzpicture}[scale=#1,line cap=butt,line join=miter]
  ...
  \draw[brandNavy,line width=1.25pt] (M)--(A);
  \draw[brandNavy,line width=1.25pt] (N)--(A);
  \draw[brandNavy,line width=1.28pt] (M)--(N);
  \draw[brandTeal,line width=1.25pt] (A)--(B);
  \draw[brandTeal,line width=1.25pt] (A)--(C);
  \draw[brandTeal,line width=1.38pt] (B)--(C);
...
% ================= PAGE 1 =================
\Header{Thalès : démontrer que des droites sont parallèles ou non}{Réciproque ou contraposée ?}
...
\fontsize{15.5}{16.5}\selectfont $\dfrac{\AMcol}{\ABcol}$
\end{minipage}
\hspace{1.0cm}
\begin{minipage}[c]{6.5cm}
\centering
{\bfseries\color{brandOrange} Sur la droite $A,N,C$}\par\vspace{0.02cm}
\fontsize{15.5}{16.5}\selectfont $\dfrac{\ANcol}{\ACcol}$
\end{minipage}
\par\vspace{0.05cm}
{\small\bfseries\color{brandNavy} Même modèle : longueur de $A$ jusqu'à la droite $(MN)$ / longueur de $A$ jusqu'à la droite $(BC)$.}
};
\end{tikzpicture}
\end{center}
\Sep
...
$\displaystyle\dfrac{\AMcol}{\ABcol}=\dfrac{\LowDotted{0.84}}{\LowDotted{0.84}}=\LowDotted{1.02}$}
};
\end{tikzpicture}
\end{minipage}\hfill
...
{\fontsize{19}{20}\selectfont
$\displaystyle\dfrac{\ANcol}{\ACcol}=\dfrac{\LowDotted{0.84}}{\LowDotted{0.84}}=\LowDotted{1.02}$}
};
\end{tikzpicture}
\end{minipage}
...
\begin{center}
\begin{tikzpicture}
\node[rounded corners=7pt,draw=brandOrange!55,fill=softOrange!38,line width=0.65pt,text width=17.95cm,inner sep=5pt,align=center] {\textbf{Pour comparer :} je simplifie, je calcule les quotients, ou j'utilise l'égalité des produits en croix. Ce n'est pas un tableau de proportionnalité déjà donné : c'est un test.};
\end{tikzpicture}
\end{center}
\Sep
...
\hspace{0.4cm}
\tikz[baseline=-0.72ex]{\node[rounded corners=7pt,draw=brandOrange!55,fill=softOrange!35,line width=0.70pt,minimum width=6.35cm,inner xsep=13pt,inner ysep=8pt,font=\bfseries\fontsize{10.8}{11.5}\selectfont\color{brandNavy}]{\BigCheckBox \raisebox{0.90ex}{réciproque} \qquad \BigCheckBox \raisebox{0.90ex}{contraposée}};}
\end{center}

\newpage

% ================= PAGE 2 =================
\Header{Fiche-guide : réciproque ou contraposée ?}{J'adapte les lettres à la figure de mon exercice}
...
\vspace{0.04cm}
\begin{tikzpicture}
\node[rounded corners=7pt,draw=brandOrange!55,fill=softOrange!38,line width=0.65pt,text width=12.60cm,inner sep=5.0pt,align=center] {\bfseries\color{brandNavy} Pour comparer : je simplifie, je calcule les quotients, ou j'utilise l'égalité des produits en croix.};
\end{tikzpicture}
...
\begin{center}
\begin{tikzpicture}
\node[rounded corners=7pt,draw=brandOrange!55,fill=softOrange!50,line width=0.65pt,text width=12.60cm,inner sep=7.0pt,align=center] {\bfseries\color{brandNavy} Ma réponse est-elle justifiée par le bon mot ? \hspace{0.22cm}\LowBigCheckBox réciproque \hspace{0.25cm}\LowBigCheckBox contraposée};
\end{tikzpicture}
\end{center}
...
\Header{Deux exemples entièrement rédigés}{Un cas emboîté et un cas papillon}

\begin{center}
\tikz\node[rounded corners=7pt,draw=brandNavy!40,fill=softBlue!20,line width=.70pt,text width=18.25cm,inner sep=5pt,align=center]{%
{\bfseries\color{brandNavy} Même démarche dans les deux configurations : je calcule deux rapports, je les compare, puis j'emploie le bon raisonnement.}
};
\end{center}

\vspace{0.12cm}
{\bfseries\color{brandTeal}\fontsize{12.6}{13.4}\selectfont Exemple 1 — cas emboîté : les droites sont parallèles}\par\vspace{0.05cm}
\begin{tikzpicture}
\node[rounded corners=9pt,draw=brandTeal,fill=brandTeal!4,line width=.85pt,text width=18.45cm,inner sep=8pt,align=left] {
\begin{minipage}[c]{6.15cm}
\centering
\FigureEmboitee{0.72}\par\vspace{0.08cm}
{\fontsize{9.4}{10.1}\selectfont
$AM=4$ cm \quad $AB=10$ cm\par
$AN=6$ cm \quad $AC=15$ cm}
\end{minipage}\hfill
\begin{minipage}[c]{11.55cm}
{\bfseries\color{brandNavy} 1. Je vérifie le contexte.}\par
Les points $A,M,B$ sont alignés. Les points $A,N,C$ sont alignés. Les points sont placés dans le même ordre : $A-M-B$ et $A-N-C$.\par\vspace{0.10cm}
{\bfseries\color{brandNavy} 2. Je calcule les deux rapports.}\par
\[
\frac{AM}{AB}=\frac{4}{10}
\qquad\text{et}\qquad
\frac{AN}{AC}=\frac{6}{15}
\]
J'utilise l'égalité des produits en croix : $4\times15=60$ et $10\times6=60$. Les deux rapports sont donc égaux.\par\vspace{0.10cm}
{\bfseries\color{brandTeal} 3. Je conclus.}\par
...
\vspace{0.35cm}
{\bfseries\color{brandOrange}\fontsize{12.6}{13.4}\selectfont Exemple 2 — cas papillon : les droites ne sont pas parallèles}\par\vspace{0.05cm}
\begin{tikzpicture}
\node[rounded corners=9pt,draw=brandOrange,fill=brandOrange!4,line width=.85pt,text width=18.45cm,inner sep=8pt,align=left] {
\begin{minipage}[c]{6.15cm}
\centering
\FigurePapillon{0.66}\par\vspace{0.08cm}
{\fontsize{9.4}{10.1}\selectfont
$AM=3$ cm \quad $AB=5$ cm\par
$AN=4$ cm \quad $AC=7$ cm}
\end{minipage}\hfill
\begin{minipage}[c]{11.55cm}
{\bfseries\color{brandNavy} 1. Je vérifie le contexte.}\par
Les points $M,A,B$ sont alignés. Les points $N,A,C$ sont alignés. Le sommet commun est $A$.\par\vspace{0.10cm}
{\bfseries\color{brandNavy} 2. Je calcule les deux rapports.}\par
\[
\frac{AM}{AB}=\frac{3}{5}
\qquad\text{et}\qquad
\frac{AN}{AC}=\frac{4}{7}
\]
J'utilise les produits en croix : $3\times7=21$ et $5\times4=20$. Les deux rapports sont donc différents.\par\vspace{0.10cm}
