...
// Lot 2b (02/09/2026) : plus aucune reprise du travail d'une autre case. Un
// code régénéré retrouve sa progression par le serveur — la fiche suit l'élève,
// pas le code — et l'ancienne case reste sur l'appareil, intacte, sans être
// proposée à personne.
test("un nouveau code arrive après un code refusé : l’ancienne case reste en place, rien n’est proposé, le nouveau code repart de ce que le serveur a", async () => {
  const serveur = fauxServeur();
  ...
  travailler(a, 2);
  await a.temps.avancer(1000);
  // Le professeur régénère le code : le serveur garde la fiche sous le code N.
  serveur.poser(N, serveur.etat(B).parcours, serveur.etat(B).revision);
  serveur.forcer = {statut: 404, donnees: {ok: false}};
  travailler(a, 3);                 // fait après la régénération : reste ici
  await a.temps.avancer(1000);
  assert.equal(a.api.sync.detache, true);
  serveur.forcer = null;
