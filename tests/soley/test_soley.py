...
() => {
  ...
  if (LV.length !== 73) fails.push(`LV.length=${LV.length} au lieu de 73`);
  const cles = new Set();
  LV.forEach((L, i) => {
    const nom = `${i}:${L.name}`;
...
def principal():
    ...
    with sync_playwright() as p:
        ...
        section("T2 solve(i) gagne pour les 73 niveaux", not perdants,
        ...
        section("T9 vieille sauvegarde (sans champ cours) chargée sans erreur ni migration",
                vieux["sansCours"] and vieux["niveaux"] == 73, "")
        ctx4.close()
        ...
        etat9 = page.evaluate("""() => ({
          ...
          rayons: document.querySelectorAll("#coursbody [data-rayon='terminal']").length,
          alignes: (() => {
            const rayons = [...document.querySelectorAll("#coursbody [data-rayon='terminal'] line")]
              .map(l => +l.getAttribute('x2'));
            const cases = [...document.querySelectorAll('#coursbody rect[data-terminal]')]
