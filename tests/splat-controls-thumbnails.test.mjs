...
test("les quatre miniatures utilisent des vues fideles et adaptees aux outils", () => {
  ...
  assert.match(cataloguePage, /catalogue-refonte-data\.js\?v=equascript-20260828-1/);
...
test("la collection Splat suit l'ordre pedagogique valide", () => {
  assert.match(catalogueScript, /path === "outils\/splat_tache_barre\.html"\) return 0;[\s\S]*path === "outils\/splat\.html"\) return 1;[\s\S]*path === "outils\/splat_equations\.html"\) return 2;[\s\S]*path === "outils\/equasplat\.html"\) return 3;[\s\S]*path === "outils\/equascript\.html"\) return 4;/);
  assert.match(catalogueScript, /collectionCardRank\(state\.collection, item\.resource\.path\)/);
  assert.match(cataloguePage, /catalogue-refonte\.js\?v=equascript-20260828-1/);
});
