...
export function dessinerFigure(description = {}) {
  ...
  const marqueAngleDroit = (i, couleur) => {
    ...
    return polyligne([p1, p2, p3], { couleur, epaisseur: 3.5 });
  };
  ...
  for (const lettre of mesuresAnglesVisibles) {
    ...
    morceaux.push(
      texte(contenu, position, { couleur: couleurAngle(i), taille: 15, halo: 4 }),
    );
  }
  ...
  for (const c of mesuresCotesVisibles) {
    ...
    const position = [
      m[0] + Math.cos(versExterieur) * 20,
      m[1] + Math.sin(versExterieur) * 20,
    ];
    morceaux.push(
...
export function dessinerCercle(description = {}) {
  ...
  const angleDe = (valeur) => {
    if (typeof valeur === "number") return -valeur * RAD; // y écran vers le bas
    const point = pointsNommes.find((p) => p.nom === valeur);
    if (!point) {
      throw new RangeError(`cercle : le point « ${valeur} » n'est pas défini`);
    }
    return -point.angleDeg * RAD;
  };
  ...
  if (visible.rayonVersDeg !== null) {
    const angle = angleDe(visible.rayonVersDeg);
