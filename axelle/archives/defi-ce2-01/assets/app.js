(function () {
  const session = window.AXELLE_SESSION || { memos: [], questions: [] };
  const screens = {
  ...
  function renderMemos() {
    memoGrid.innerHTML = session.memos.map((memo) => `
  ...
  function renderQuestion() {
    ...
    nextButton.querySelector("span:first-child").textContent = questionIndex === session.questions.length - 1 ? "Voir mon résultat" : "Question suivante";
  }
  ...
  function chooseAnswer(button) {
    ...
    feedback.innerHTML = `<strong>${correct ? "Bien joué !" : "Presque !"}</strong>${question.explanation}`;
  ...
  function finish() {
    ...
    finalCount.textContent = String(session.questions.length);
    if (score === session.questions.length) {
      finishMessage.textContent = "Tout est juste. Tes souvenirs de CE2 sont bien solides !";
    } else if (score >= Math.ceil(session.questions.length * 0.6)) {
      finishMessage.textContent = "Très joli parcours. Les explications t’ont permis de revoir les points importants.";
    } else {
      finishMessage.textContent = "Tu es allée jusqu’au bout, et c’était le plus important pour ce premier essai. Tu peux rejouer tranquillement.";
    }
    showScreen("finish");
  }
  ...
  hintButton.addEventListener("click", () => {
    ...
    hintButton.textContent = hintBox.hidden ? "Afficher un indice" : "Masquer l’indice";
  });
